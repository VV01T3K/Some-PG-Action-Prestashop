  ...
  <section id="search_filters" class="js-search-filters" data-testid="grid-filters-desktop">
    <div class="flex flex-col gap-6">
      ...
      {foreach from=$displayedFacets item="facet"}
        {if $facet.label != 'Selections'}
          {assign var=_expand_id value=10|mt_rand:100000}
          {assign var=_collapse value=true}
          {foreach from=$facet.filters item="filter"}
            {if $filter.active}{assign var=_collapse value=false}{/if}
          {/foreach} <div role="group" aria-labelledby="filter_group_{$_expand_id}" data-testid="grid-filters-group"
            data-type="{$facet.type}" data-name="{$facet.label}" class="filter-group">

            {* Filter group title *}
            <h3 id="filter_group_{$_expand_id}" class="text-base text-dark-blue-500 mb-4 font-bold"
                  ...
                  {foreach from=$facet.filters key=filter_key item="filter"}
                    ...
                    <li>
                      ...
                      {else}
                        ...
                        <label class="group flex items-center gap-2 cursor-pointer" for="facet_input_{$_expand_id}_{$filter_key}">
                          {* Checkbox/Radio *}
                          <span class="relative flex items-center justify-center h-5 w-5 rounded border border-neutral-300 bg-white
                            ...
                            {elseif isset($filter.properties.texture)}
                              <span class="w-full h-full rounded"
                                style="background-image:url({$filter.properties.texture}); background-size: cover;"></span>
                            {else}
            ...
            {elseif $facet.widgetType == 'dropdown'}
              {block name='facet_item_dropdown'}
                <div class="relative">
                  ...
                  <div
                    class="dropdown-menu absolute z-10 w-full mt-1 bg-white border border-neutral-300 rounded-md shadow-lg hidden">
                    {foreach from=$facet.filters item="filter"}
                      {if !$filter.active}
            ...
            {elseif $facet.widgetType == 'slider'}
              {block name='facet_item_slider'}
                {foreach from=$facet.filters item="filter"}
                  <ul id="facet_{$_expand_id}" class="faceted-slider" data-slider-min="{$facet.properties.min}"
